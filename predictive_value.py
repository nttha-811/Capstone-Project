...
for t in range(initial_train_size, len(y)):
    ...
    X_gas_m_tr = X_gas_monthly[:t]
    X_rate_m_tr = X_rate_monthly[:t]

    y_te = y[t:t+1]
    y_lag1_te = y_lag1[t:t+1]
    X_gas_w_te = X_gas_weekly[t:t+1]
    X_rate_w_te = X_rate_weekly[t:t+1]
    X_gas_m_te = X_gas_monthly[t:t+1]
    X_rate_m_te = X_rate_monthly[t:t+1]
    ...
    # Monthly OLS
    A_ols = np.vstack([np.ones(len(y_tr)), X_gas_m_tr, X_rate_m_tr]).T
    coeff_ols, _, _, _ = np.linalg.lstsq(A_ols, y_tr, rcond=None)
    pred_ols = coeff_ols[0] + coeff_ols[1] * X_gas_m_te[0] + coeff_ols[2] * X_rate_m_te[0]
    preds_ols_monthly.append(pred_ols)

    # Univariate MIDAS (Gasoline)
    ...
    # Bivariate MIDAS (Gasoline + Policy Rate)
...
mse_ar1 = mean_squared_error(actuals, preds_ar1)

mae_ols = mean_absolute_error(actuals, preds_ols_monthly)
mse_ols = mean_squared_error(actuals, preds_ols_monthly)

mae_midas_gas = mean_absolute_error(actuals, preds_midas_gas)
mse_midas_gas = mean_squared_error(actuals, preds_midas_gas)

mae_midas_bi = mean_absolute_error(actuals, preds_midas_bivariate)
mse_midas_bi = mean_squared_error(actuals, preds_midas_bivariate)

impr_gas = ((mae_ar1 - mae_midas_gas) / mae_ar1) * 100
impr_bi = ((mae_ar1 - mae_midas_bi) / mae_ar1) * 100

print("Out-of-Sample Nowcast Accuracy Summary:")
print(f"{'Model':<35} | {'Frequency':<18} | {'OOS MAE':<10} | {'OOS MSE':<12} | {'Gain vs AR(1)':<10}")
print("-" * 88)
print(f"{'AR(1) Baseline':<35} | {'Monthly Only':<18} | {mae_ar1:.6f}   | {mse_ar1:.8e}   | Baseline")
print(f"{'Monthly OLS (Gas + Rate)':<35} | {'Monthly Aggregated':<18} | {mae_ols:.6f}   | {mse_ols:.8e}   | {((mae_ar1-mae_ols)/mae_ar1)*100:+.2f}%")
print(f"{'Univariate MIDAS (Weekly Gas)':<35} | {'Weekly High-Freq':<18} | {mae_midas_gas:.6f}   | {mse_midas_gas:.8e}   | {impr_gas:+.2f}%")
print(f"{'Bivariate MIDAS (Gas + Rate)':<35} | {'Weekly High-Freq':<18} | {mae_midas_bi:.6f}   | {mse_midas_bi:.8e}   | {impr_bi:+.2f}%")
