...
def run_split_commodities_midas():
    split_path = (
        "cleaned_predictors/commodity_prices_split.csv"
        if os.path.exists("cleaned_predictors/commodity_prices_split.csv")
        else "predictors/commodity_prices_split.csv"
    )
    cpi_path = (
    ...
    df_energy = load_weekly_lag_matrix(split_path, "diff_log_energy")
    df_non_energy = load_weekly_lag_matrix(split_path, "diff_log_non_energy")
    ...
    df_merged = df_energy.join(
        df_non_energy, lsuffix="_en", rsuffix="_ne", how="inner"
    )
    ...
    X1 = df_merged[["Lag_0_en", "Lag_1_en", "Lag_2_en", "Lag_3_en"]].values
    X2 = df_merged[["Lag_0_ne", "Lag_1_ne", "Lag_2_ne", "Lag_3_ne"]].values
    y = df_merged["inflation"].values

    initial_guess = [0.001, 0.04, 0.01, -0.1, -0.05, -0.1, -0.05]
    ...
    print("Bivariate MIDAS (Energy + Non-Energy Commodities):")
    print(f"Alpha:                 {alpha:.6f}")
    print(f"Beta 1 (Energy):       {beta1:.6f}")
    print(f"Beta 2 (Non-Energy):   {beta2:.6f}\n")

    print(f"{'Lag':<6} {'Energy Weight':<16} {'Non-Energy Weight':<16}")
    print("-" * 40)
    for i in range(4):
        print(f"t-{i:<4} {w1_opt[i]:<16.4f} {w2_opt[i]:<16.4f}")
